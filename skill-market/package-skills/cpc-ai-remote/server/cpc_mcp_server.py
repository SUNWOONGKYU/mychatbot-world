...
def _tool_wait_cpc_command(platoon_id: str, timeout: int = 120) -> dict | None:
    """CPC + Tailscale Bridge에서 PENDING 명령 폴링. Bridge 우선, CPC 폴백."""
    deadline = time.time() + timeout
    while time.time() < deadline:
        # 1) Tailscale HTTP Bridge 확인 (localhost:8443)
        try:
            bridge_cmd = _bridge_get_command()
            if bridge_cmd:
                return {
                    'cmd_id': bridge_cmd.get('id', ''),
                    'text': bridge_cmd.get('text', ''),
...
def _bridge_get_command():
    """Tailscale HTTP Bridge에서 PENDING 명령 가져오기 (localhost:8443)."""
    req = urllib.request.Request('http://localhost:8443/command')
...
def _bridge_report_result(cmd_id: str, result: str):
    """Tailscale HTTP Bridge에 결과 보고."""
    body = json.dumps({'cmd_id': cmd_id, 'result': result}).encode('utf-8')
    req = urllib.request.Request(
        'http://localhost:8443/result',
        data=body,
        headers={'Content-Type': 'application/json'},
        method='POST',
    )
...
def _tool_report_cpc_result(cmd_id: str, result: str) -> dict:
    """CPC + Tailscale Bridge에 명령 처리 결과를 보고."""
    # Tailscale Bridge에도 결과 보고 (ts- 접두사면 Bridge 명령)
    if cmd_id.startswith('ts-'):
        try:
            _bridge_report_result(cmd_id, result)
        except Exception:
            pass
        return {'ok': True, 'response': {'id': cmd_id, 'status': 'DONE', 'result': result}}
    # CPC(Supabase) 보고
    try:
