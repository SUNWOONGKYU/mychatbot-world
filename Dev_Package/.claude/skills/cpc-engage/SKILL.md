...
# 실행 중이 아니면 백그라운드로 시작
cd {프로젝트경로}/cpc-agent-server && python server.py &
```
...
```bash
SESSION_ID=$(ls -t ~/.claude/projects/*{프로젝트폴더}*/*.jsonl 2>/dev/null | head -1 | xargs basename | sed 's/.jsonl//')
```
...
curl -s -X PATCH "https://claude-platoons-control.vercel.app/api/platoons/{소대키}/status" \
  -H "Content-Type: application/json" \
  -d '{"session_url": "https://claude.ai/code/xxxxxxxx"}'
```
