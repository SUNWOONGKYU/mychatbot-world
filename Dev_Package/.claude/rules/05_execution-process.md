...
## 0단계: F Task 실행 전 선행 조건 확인 ⭐ 재발방지 규칙

> **배경**: Skills·Learning Task에서 DB/API 없이 JS 하드코딩으로 구현 후 완료 처리한
> 결함 재발 방지. F(프론트) Task는 반드시 아래를 확인 후 실행한다.

**F(프론트) Task 실행 전 의무 체크리스트:**
...
│      → 없으면: DB Task 먼저 실행 후 F Task 진행              │
│                                                             │
│ □ 2. 이 페이지 데이터를 제공하는 API가 존재하는가?            │
│      → 없으면: BA Task 먼저 실행 후 F Task 진행              │
│                                                             │
