...
## 0-1. ⛔ 데이터 소스 검증 (F/BA Task 필수) ⭐ 재발방지 규칙

> **배경**: Skills(S3F12)·Learning(S3F14) Task에서 Supabase 연결 없이 JS 하드코딩으로
> 구현하고 "완료" 처리한 결함이 발생함. 이를 재발 방지하기 위한 필수 검증 항목.

### F(프론트) Task 검증 시 데이터 소스 확인 필수

```
🚫 JS 파일 내 배열/객체로 도메인 데이터가 정의되어 있으면 → FAIL
🚫 API 호출 없이 window.MCW.xxx 전역 변수에서만 데이터를 가져오면 → FAIL
✅ fetch('/api/Backend_APIs/xxx') 또는 supabase.from('xxx') 호출이 있어야 → PASS
```

**검증 질문 (F Task 완료 전 반드시 답해야 함):**

| # | 질문 | 합격 기준 |
|---|------|---------|
| 1 | 이 페이지의 핵심 데이터는 어디서 오는가? | Supabase API fetch |
| 2 | JS 파일 내에 도메인 데이터 배열이 하드코딩되어 있는가? | 없어야 함 |
| 3 | 선행 DB Task가 완료되어 해당 테이블이 존재하는가? | 존재해야 함 |
| 4 | 선행 BA(API) Task가 완료되어 엔드포인트가 존재하는가? | 존재해야 함 |

**위 4개 중 하나라도 FAIL이면 F Task는 Verified 처리 불가.**

### BA(API) Task 검증 시 확인 필수

| # | 질문 | 합격 기준 |
|---|------|---------|
| 1 | API가 호출하는 Supabase 테이블이 실제로 존재하는가? | 존재해야 함 |
| 2 | 테이블에 테스트 가능한 mock 데이터가 있는가? | 최소 1건 이상 |
