 ...
 *   - 시나리오 2: 챗봇 검색 → 상세 보기 → 고용 요청 → 매칭 결과 확인
...
describe('시나리오 2: 챗봇 검색 → 상세 보기 → 고용 요청 → 매칭 결과 확인', () => {
  it('Step 1: 챗봇 검색 키워드로 결과 반환', async () => {
  ...
  it('Step 2: 챗봇 상세 보기 — botId로 상세 정보 반환', async () => {
...
describe('시나리오 4: 메뉴 네비게이션 (5대 메뉴 전환)', () => {
  ...
  it('챗봇 마켓 메뉴 네비게이션 — path = "/chatbots"', async () => {
...
describe('시나리오 5: 비로그인 사용자 제한 기능 확인', () => {
  ...
  it('비로그인 사용자도 챗봇 목록/검색은 접근 가능해야 한다', async () => {
    const session = new SessionStore();
...
function printSummary() {
  ...
  console.log('  시나리오 2: 챗봇 검색 → 상세 보기 → 고용 요청 → 매칭 결과');
  console.log('  시나리오 3: 커뮤니티 글 작성 → 댓글 → 좋아요 → 신고');
  console.log('  시나리오 4: 5대 메뉴 네비게이션 (홈/챗봇/구직/커뮤니티/학교)');
