...
export async function GET(request: NextRequest): Promise<NextResponse> {
  ...
  // 크리에이터(소유자) 정보 일괄 조회
  const ownerIds = [...new Set(settings.map((s) => s.owner_id))];
  ...
  const ownerMap = new Map<string, OwnerRow>((owners ?? []).map((o) => [o.id, o]));

  // 각 피상속 건의 페르소나 설정 조회 (chatbots 테이블 join)
  const inheritanceIds = settings.map((s) => s.id);
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  const { data: setting, error: settingError } = await supabase
    ...
    .eq('id', inheritanceId)
    .or(`heir_id.eq.${userId},heir_email.eq.${userEmail}`)
    .maybeSingle<InheritanceSetting>();
