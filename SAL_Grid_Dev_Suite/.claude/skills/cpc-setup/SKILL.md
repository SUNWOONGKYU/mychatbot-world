...
## Phase 6: MCP 서버 등록 + 권한 설정 (소대장 원격 운용 필수)

이 단계는 **각 PC의 Claude Code가 CPC 명령을 수신하고, y/n 프롬프트 없이 자율 실행**할 수 있도록 설정한다.

### 6-1. CPC MCP 서버 등록

`~/.claude/settings.json`에 `mcpServers.cpc` 항목을 추가한다.
기존 설정을 보존하면서 병합해야 한다.

**MCP 서버 소스 파일 위치 확인:**
```bash
# cpc_mcp_server.py 위치 찾기
find ~ -name "cpc_mcp_server.py" 2>/dev/null | head -3
# 또는 프로젝트 내
ls ./cpc-agent-server/cpc_mcp_server.py 2>/dev/null
```
...
```json
{
  "mcpServers": {
    "cpc": {
      "command": "python3",
      "args": ["<cpc_mcp_server.py 절대경로>"]
    }
  }
}
```

**또는 cpc-channel 방식 (Bun 기반):**
```json
{
  "mcpServers": {
    "cpc-channel": {
      "command": "bun",
      "args": ["run", "<cpc-channel/dist/cpc-channel.js 절대경로>"]
    }
  }
}
```

> 기존 `mcpServers`가 있으면 병합, 없으면 새로 생성.
> settings.json의 다른 항목(env, statusLine, enabledPlugins 등)은 절대 건드리지 않는다.

### 6-2. bypassPermissions 설정

현재 프로젝트의 `.claude/settings.local.json`에 권한 설정을 추가한다.
이렇게 하면 **CPC 명령 처리 중 y/n 프롬프트가 뜨지 않아** 원격 운용이 가능하다.
...
> 기존 `.claude/settings.local.json`이 있으면 `permissions` 항목만 병합/덮어쓰기.
> `hooks` 등 다른 항목은 보존.
...
with open('$HOME/.claude/settings.json') as f:
    s = json.load(f)
mcp = s.get('mcpServers', s.get('mcp', {}).get('servers', {}))
if 'cpc' in mcp or 'cpc-channel' in mcp:
    print('✅ CPC MCP 서버 등록됨')
else:
    print('❌ CPC MCP 서버 미등록')
"

# bypassPermissions 확인
python3 -c "
import json, os
...
if os.path.exists(path):
    ...
    if mode == 'bypassPermissions':
        print('✅ bypassPermissions 설정됨')
    else:
        print(f'❌ defaultMode = {mode}')
else:
    print('❌ settings.local.json 없음')
"
```

> **중요:** MCP 서버 등록은 Claude Code **재시작 후** 적용된다.
> 설정 변경 후 사용자에게 "Claude Code를 재시작한 뒤 `/cpc-engage`를 실행하세요" 안내.
...
[ ] CPC MCP 서버 등록 (~/.claude/settings.json → mcpServers.cpc)
[ ] bypassPermissions 설정 (.claude/settings.local.json)
