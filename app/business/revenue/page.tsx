...
function downloadCsv(items: RevenueItem[]) {
  ...
  const rows = items.map((i: any) => [
    i.date,
    i.amount.toString(),
    (SOURCE_LABELS as any)[i.source]?.label ?? i.source,
    i.persona_name,
    i.description ?? '',
  ]);
  const csv = [header, ...rows]
    .map((r: any) => r.map((v: any) => `"${v.replace(/"/g, '""')}"`).join(','))
    .join('\n');
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap gap-3 items-end">
        ...
        <div className="flex flex-col gap-1">
          ...
          <select
            value={source}
            onChange={(e: any) => setSource(e.target.value as RevenueSource | '')}
            className="rounded-lg border border-border bg-bg-subtle px-3 py-2 text-sm text-text-primary focus:outline-none focus:ring-2 focus:ring-primary/40"
          >
