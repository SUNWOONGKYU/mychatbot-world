...
export default function SettlementPage() {
  ...
  const handleSetDefault = async (id: string) => {
    try {
      ...
      setMethods((prev) =>
        prev.map((m: any) => ({ ...m, is_default: m.id === id })),
      );
    } catch {
      alert('설정에 실패했습니다.');
    }
    ...
    <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center gap-6">
          ...
          <div className="flex flex-col gap-3 flex-shrink-0">
            ...
            <div className="flex items-center gap-2">
              ...
              <select
                value={selectedCycle}
                onChange={(e: any) => handleCycleChange(e.target.value as SettlementCycle)}
      ...
      <div>
        ...
        <div className="space-y-3">
          {loadingMethods ? (
            [...Array(2)].map((_: any, i: any) => <Skeleton key={i} className="h-16 w-full rounded-xl" />)
          ) : methods.length === 0 ? (
