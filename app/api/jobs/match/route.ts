...
function calcLocalSkillScore(requiredSkills: string[], applicantSkills: string[]): number {
  ...
  const required = requiredSkills.map((s: any) => s.toLowerCase().trim());
  const provided = applicantSkills.map((s: any) => s.toLowerCase().trim());

  const matched = required.filter((s: any) => provided.includes(s)).length;
...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    const matchInserts = sortedResults.map((r: any) => ({
      job_id,
