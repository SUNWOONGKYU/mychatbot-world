            ...
            {([['dark', '🌙', '다크 모드'], ['light', '☀️', '라이트 모드']] as const).map(([mode, icon, label]) => (
              <button
                ...
                style={{
                  ...
                  gap: '8px',
                  padding: '16px',
                  background: themeMode === mode ? 'rgba(99,102,241,0.15)' : 'rgba(255,255,255,0.03)',
                  border: `2px solid ${themeMode === mode ? '#6366f1' : 'rgba(255,255,255,0.08)'}`,
                  borderRadius: '12px',
                  color: themeMode === mode ? 'white' : 'rgba(255,255,255,0.6)',
                  cursor: 'pointer',
                  fontSize: '0.9rem',
                  transition: 'all 0.2s',
                }}
            ...
            {Object.entries(THEME_COLORS).map(([key, hex]) => (
              <div
                ...
                style={{
                  ...
                  background: hex,
                  cursor: 'pointer',
                  border: `3px solid ${themeColor === key ? 'white' : 'transparent'}`,
                  boxShadow: themeColor === key ? '0 0 0 3px rgba(255,255,255,0.3)' : 'none',
                  display: 'flex',
                  alignItems: 'center',
