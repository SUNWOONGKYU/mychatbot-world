...
function StatusBadge({ active }: { active: boolean }) {
  return (
    <span style={{
      ...
      background: active ? 'rgba(34,197,94,0.15)' : 'rgb(var(--bg-muted))',
      color: active ? '#22c55e' : 'rgb(var(--text-muted))',
      border: active ? '1px solid rgba(34,197,94,0.3)' : '1px solid rgb(var(--border))',
    }}>{active ? '활성' : '미설정'}</span>
  );
}
      ...
      <div style={{ ...S.card, marginBottom: '1.5rem' }}>
        <div style={{ ...S.sectionHeader, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            ...
            <p style={{ color: 'rgb(var(--text-muted))', fontSize: '0.85rem', marginTop: 4 }}>ElevenLabs API 연동 · ₩50,000/월</p>
          </div>
          <StatusBadge active={!!elevenLabsKey} />
        </div>

        <p style={{ color: 'rgb(var(--text-secondary-rgb))', fontSize: '0.9rem', marginBottom: '1rem' }}>
        ...
        <div style={{ marginBottom: '1rem' }}>
          ...
          <label style={{
            border: '2px dashed rgb(var(--border))', borderRadius: 12,
          ...
          {voiceFileName && <div style={{ fontSize: '0.8rem', color: 'rgb(var(--text-muted))', marginTop: '0.5rem' }}>선택된 파일: {voiceFileName}</div>}
        </div>
      ...
      <div style={{ ...S.card, marginBottom: '1.5rem' }}>
        <div style={{ ...S.sectionHeader, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            ...
            <p style={{ color: 'rgb(var(--text-muted))', fontSize: '0.85rem', marginTop: 4 }}>Ready Player Me API 연동 · ₩30,000/월</p>
          </div>
        ...
        <div style={{ display: 'flex', gap: 10 }}>
          <button style={{ padding: '8px 16px', background: 'rgb(var(--bg-muted))', color: 'rgb(var(--text-secondary-rgb))', border: '1px solid rgb(var(--border))', borderRadius: 10, cursor: 'pointer', fontWeight: 600, fontSize: '0.85rem' }}
      ...
      <div style={S.card}>
        <div style={{ ...S.sectionHeader, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            ...
            <p style={{ color: 'rgb(var(--text-muted))', fontSize: '0.85rem', marginTop: 4 }}>CSS 에디터 직접 편집 · ₩20,000/월</p>
          </div>
        ...
        <div style={{ marginBottom: '1rem' }}>
          ...
          <textarea
            style={{ ...darkInput, resize: 'vertical', minHeight: 200, fontFamily: 'monospace', fontSize: '0.85rem', color: 'rgb(var(--text-primary-rgb))' }}
            value={customCss}
